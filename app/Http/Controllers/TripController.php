    ...
    private function isTurnAhead($currentLocation, $turnLocation, $userBearing)
{
    ...
    public function getActiveTrip(Request $request)
    {
        ...
        if($vehicle_id){
            ...
            ->where('id',  $vehicle_id->vehicle_id)
            ->first();
            ...
            $fuelStations = FuelStation::where('trip_id', $trip->id)->get()->map(function ($station) {
                $response = [
                    ...
                    "lastprice" => (float) $station->lastprice,
                    "price" => (float)$station->price,
                    "discount" => (float)$station->discount,
                    "IFTA_tax" =>(float) $station->ifta_tax,
                    "address" => $station->address,
                    "distanceFromStart" => (float) number_format($station->distanceFromStart, 6, '.', ''), // Ensure 6 decimal places
                    "gallons_to_buy" => $station->gallons_to_buy =="0" ? null : (float) $station->gallons_to_buy,
                ];
    ...
    private function findMatchingRecords(array $decodedPolyline, array $ftpData)
{
    ...
    private function findNearestPoint($location, $polyline)
    {
        ...
        $nearestIndex = 0;
        $threshold = 50; // Max 50 miles from route
        ...
        // If station is too far from entire route, return invalid
        return ($minDistance <= $threshold) ? $nearestIndex : -1;
    }
    ...
    private function calculatePolylineDistance($userLocation, $destination, $polyline)
    {
        ...
        // If either point is too far from route, return null
